...
const TopCustomerTable = ({ customers, loading, error }) => {
  ...
  const [selectedCustomer, setSelectedCustomer] = useState(null); // State để lưu khách hàng đã chọn
  ...
  const handleSelectCustomer = (customer) => {
    setSelectedCustomer(customer); // Cập nhật state với khách hàng đã chọn
    console.log('Selected Customer:', customer); // Hoặc thực hiện hành động khác với customer đã chọn
  };
          ...
          <Tbody color={textColor}>
            {(searchResults.length > 0 ? searchResults : customers).map((customer) => (
              <Tr key={customer.id}>
                ...
                <Td>
                  <Button
                    onClick={() => handleSelectCustomer(customer)} // Gọi hàm khi nhấn nút
                    colorScheme="blue"
                    size="sm"
                  >
                    Select
